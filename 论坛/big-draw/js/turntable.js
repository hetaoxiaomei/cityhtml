    ...
    $.fn.WheelSurf = function (options) {
        ...
        return {
            init: function (angelTo) {
                ...
                ctx.arc(0, 0, 134, 0, 2 * Math.PI) //243
                ctx.stroke();
                // 画里圆
                ctx.beginPath();
                ctx.lineWidth = 23;
                ctx.strokeStyle = _default.innerCircle.color;
                ctx.arc(0, 0, 126, 0, 2 * Math.PI) //218
                ctx.stroke();
                ...
                for (var i = 0; i < num; i++) {
                    ctx.beginPath();
                    ctx.lineWidth = 120; //208
                    ctx.strokeStyle = colors[i % colors.length]
                    ctx.arc(0, 0, 60, startAngel, endAngel) //104
                    ctx.stroke();
                    startAngel = endAngel
                    endAngel += angel
                }
                ...
                $.when(loadImg()).done(function (awardPic) {
                    ...
                    for (var i = 0; i < num; i++) {
                        ...
                        ctx.drawImage(awardPic[i],25,25,50,50);  // (awardPic[i], -48, -48 - 130) 
